package com.freshman.freshmanbackend.domain.product.service.query.review;
...
import com.freshman.freshmanbackend.domain.product.dto.response.ReviewCommentResponse;
...
public class ReviewListService {
    ...
    public NoOffsetPageResponse getReviewsByProductSeq(Long productSeq, int page) {
        List<ProductReviewResponse> reviews = reviewListDao.selectPage(page, PAGE_SIZE, productSeq,
                ReviewSortType.NEWEST.getCode());
    ...
    public NoOffsetPageResponse getReviewCommentsByReviewSeq(Long reviewSeq, int page) {
        List<ReviewCommentResponse> comments = reviewCommentListDao.getReviewCommentsByReviewSeq(reviewSeq, page,
                PAGE_SIZE);
    ...
    public NoOffsetPageResponse getReviewCommentsByParentCommentSeq(Long commentSeq, int page) {
        List<ReviewCommentResponse> comments = reviewCommentListDao.getReviewCommentsByParentComment(commentSeq, page,
                PAGE_SIZE);
        if (comments.size() == PAGE_SIZE + 1) {
            comments.remove(PAGE_SIZE);
            return new NoOffsetPageResponse(comments, false);
